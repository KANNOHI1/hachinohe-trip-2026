...
### バッチ（HKがまとめて指示・後で一括実装する）
- **Itinerary を2人の合流ドラマ構成に**: 奈緒美=札幌 稲積公園駅発 / 宏勇=江東区 東雲駅発、2本のタイムラインが 9/20 13:00 三沢で合流する見せ方
- **Itinerary をプラン A/B/C タブで切替**: A/B/C をポチで切り替えてそれぞれの日別タイムラインを表示
- **A/B/C をフラット提示に**: 現状 Plan C を「推し」にしているが、今は推しを出さず A/B/C を対等に検討できる形へ（Home比較・各所の「推し」表現を中立化）
